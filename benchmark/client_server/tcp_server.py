...
SERVER_START_CMD_PATTERN = "./RTSPServer --trial={} --out-width={} --out-height={}" \
                           " --vbv-bufsize={} --udp={} --bitrate={} --out-fps={}"

# client benchmark start command pattern
CLIENT_CMD_PATTERN = 'openRTSP -V -f {} -w {} -h {} -Q 600 -n -d {} -F trial_{}_<replace>_ ' \
                     'rtsp://10.42.0.1:8554/<replace>'

PIDSTAT_CMD_PATTERN = 'pidstat -u -r -p {} 15'


# sets socket options to keep connection alive
def set_keep_alive(sock, after_idle_sec=30, interval_sec=30, max_fails=5):
...
def wait_for_data(sock, sleep_sec=2):
    data = ''
    while data == '':
...
if __name__ == "__main__":
    ...
    # open the log file
    with open('server_output.log', 'w') as logfile:

        with open('pidstat.log', 'w') as pidstat_logfile:
            ...
            for trial in trials['benchmark']:
                ...
                try:
                    ...
                    video_server_process = subprocess.Popen(server_cmd.split(),
                                                            stdout=logfile,
                                                            stderr=logfile)
                    ...
                    print "Video server is ready to serve clients: {}".format(video_server_process.pid)
                    ...
                    print "Start pidstat utility ..."

                    pidstat_cmd = PIDSTAT_CMD_PATTERN.format(video_server_process.pid)

                    pidstat_process = subprocess.Popen(pidstat_cmd.split(),
                                                       stdout=pidstat_logfile,
                                                       stderr=pidstat_logfile)

                    print "Waiting for client's cmd to stop the server ..."

                    client_data = wait_for_data(client_connection)

                    pidstat_process.terminate()
                    pidstat_process.wait()

                    # stop the server
                    video_server_process.terminate()

                    exit_code = video_server_process.wait()
    ...
    print 'Finishing benchmark... Sending KILL signal...'
